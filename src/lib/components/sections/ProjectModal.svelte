<script lang="ts">
  ...
  const imgs = (p: StrapiEntity<ProjectDTO> | null) => p?.gallery ?? [];
</script>
...
>
  ...
  {@const p = project}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 lg:gap-8 lg:items-start">
      <div class="order-2 lg:order-1 lg:col-span-3 lg:h-[calc(100vh-6rem)] lg:overflow-y-auto lg:pr-2 scrollbar-hidden">
      ...
      <aside class="order-1 lg:order-2 lg:col-span-1 overflow-anchor-none">
        <div class="mb-12 lg:mb-0">
            ...
            {#if p.service}
                <div class="mt-6">
                <span class="inline-flex items-center rounded-full bg-bright-gold-500 px-4 py-2 text-sm font-bold tracking-wide text-graphite">
                    {p.service}
                </span>
                </div>
            {/if}
